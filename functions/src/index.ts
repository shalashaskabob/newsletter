...
app.post('/generate-image', async (req, res): Promise<void> => {
  try {
    ...
    if (!screenshotOneApiKey) {
      ...
      res.status(500).json({ error: 'ScreenshotOne API key not configured' });
      return;
    }
...
function generateNewsletterHTML(newsletterData: any): string {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="newsletter-container">
        ...
        <main class="newsletter-content">
            ${newsletterData.sections.map((section: any) => `
                <section class="newsletter-section">
                    <h2 class="section-title">${section.title}</h2>

                    ${section.traderTrades ? `
                        <div class="trades-grid">
                            ${section.traderTrades.map((trade: any) => `
...
export const newsletterApi = onRequest({
  timeoutSeconds: 300,
  memory: '1GiB',
  maxInstances: 10
}, app);
